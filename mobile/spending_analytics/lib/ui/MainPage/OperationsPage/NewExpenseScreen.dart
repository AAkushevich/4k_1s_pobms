...
class NewExpenseScreen extends StatefulWidget {
  ...
  NewExpenseScreen(this._sharedPrefRepository, this.addNewExpense, this.categories, this.accounts);
  ...
  final Function(DateTime dateTime, double amount, int accountId, int categoryId) addNewExpense;

  @override
  State<StatefulWidget> createState() {
    return NewExpenseScreenState(_sharedPrefRepository, addNewExpense, accounts, categories);
  }

}

class NewExpenseScreenState extends State<NewExpenseScreen> {
  ...
  NewExpenseScreenState(this._sharedPrefRepository, this.addNewExpense, this.accounts, this.categories){
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text("Новый расход", style: TextStyle(color: Colors.black)),
        leading: IconButton(
            ...
            child: Text("Сохранить", style: TextStyle(color: Colors.black)),
              onPressed: () async{
                if(balanceController.text.isNotEmpty && chosenAccountId != 0 && chosenCategoryId != 0) {
                  await addNewExpense(dateTime, double.parse(balanceController.text),
          ...
          Container(
            ...
            child: GridView.builder(
              ...
              itemBuilder: (BuildContext context, int index) {
                if(categories[index].categoryType.compareTo("expense") == 0) {
                  return Container(
                      child: Column(
